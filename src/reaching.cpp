...
ReachingValues::ReachingValues(const SsaFunction &fn) : fn_(fn) {
  ...
  for (int i = 0; i < fn.value_count(); ++i) {
    ...
    if (value.is_live_in()) entry_[cfg.entry][value.storage] = const_cast<SsaValue *>(&value);
  }
  ...
  while (!stack.empty()) {
    ...
    for (const SsaOp *phi : fn[b].phis)
      if (phi->out != nullptr) state[phi->out->storage] = phi->out;
    for (const SsaOp *op : fn[b].ops)
      if (op->out != nullptr) state[op->out->storage] = op->out;
...
SsaValue *ReachingValues::before(const SsaOp &op, const Storage &storage) const {
  SsaValue *value = at_entry(op.block, storage);

  for (const SsaOp *phi : fn_[op.block].phis) {
    if (phi == &op) return value;
    if (phi->out != nullptr && phi->out->storage == storage) value = phi->out;
  }
  for (const SsaOp *earlier : fn_[op.block].ops) {
    if (earlier == &op) return value;
    if (earlier->out != nullptr && earlier->out->storage == storage) value = earlier->out;
  }
