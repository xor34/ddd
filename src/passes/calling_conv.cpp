...
public:
  ...
  std::string description() const override {
    return "annotate calls with their arguments, and the entry with its parameters";
  }
  ...
  void run(SsaFunction &fn, PassContext &ctx) override {
    ...
    if (ctx.verbose) ctx.stream() << "  annotated " << calls << " call(s)\n";
  }
...
private:
  ...
  static int annotate_calls(SsaFunction &fn, const PassContext &ctx,
    ...
    fn.for_each_op([&](SsaOp &op) {
      if (op.opc != ghidra::CPUI_CALL && op.opc != ghidra::CPUI_CALLIND) return;
      ++calls;
      ...
      for (size_t i = 0; i < registers.size(); ++i) {
        ...
        if (value == nullptr) continue; // register never mentioned at all
  ...
  static const SsaValue *find_live_in(const SsaFunction &fn, const Storage &storage) {
    for (int i = 0; i < fn.value_count(); ++i) {
      const SsaValue &value = fn.value(i);
      if (value.is_live_in() && value.storage == storage) return &value;
    }
    return nullptr;
  }
};
