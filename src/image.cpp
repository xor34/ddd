...
std::optional<uint64_t> Image::read_int(uint64_t addr, unsigned size) const {
  if (size == 0 || size > 8 || !contains(addr, size)) return std::nullopt;
...
std::optional<std::string> Image::read_string(uint64_t addr, size_t max_length) const {
  if (!contains(addr)) return std::nullopt;

  std::string text;
  for (uint64_t cursor = addr; cursor < limit() && text.size() <= max_length; ++cursor) {
    uint8_t byte = *at(cursor);
    if (byte == 0) return text.empty() ? std::nullopt : std::optional<std::string>(text);
