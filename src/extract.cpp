...
void ExtractorRegistry::add(Entry entry) {
  entries_.push_back(std::move(entry));
}

std::unique_ptr<Extractor>
ExtractorRegistry::create(const std::string &name) const {
...
std::vector<Finding> extract(const Image &image, ExtractContext &ctx,
  ...
  for (const ExtractorRegistry::Entry &entry :
       ExtractorRegistry::instance().entries()) {
    if (!names.empty() &&
...
std::string to_string(const Finding &finding) {
  ...
  if (!finding.suggested_spec.empty())
    os << "  [--region spec: " << finding.suggested_spec << "]";
  return os.str();
}
