...
Pattern make_pattern(Pattern::Kind kind, OpCode opc, uint64_t constant, int slot,
                     std::vector<Pattern> ins, bool commutative) {
  Pattern pattern;
  pattern.kind_ = kind;
  pattern.opc_ = opc;
...
bool Pattern::match_operands(const SsaOp &op, Match &m) const {
  for (size_t i = 0; i < ins_.size(); ++i)
    if (!ins_[i].match(op.ins[i], m)) return false;
  return true;
}

bool Pattern::match(const SsaOp &op, Match &m) const {
  if (kind_ != Kind::Op) return false;
  if (op.opc != opc_ || op.ins.size() != ins_.size()) return false;
  ...
  if (!commutative_ || ins_.size() != 2) return false;
...
Pattern any() { return make_pattern(Pattern::Kind::Any, ghidra::CPUI_COPY, 0, -1, {}, false); }

Pattern val(int slot) {
  return make_pattern(Pattern::Kind::Value, ghidra::CPUI_COPY, 0, slot, {}, false);
}

Pattern imm(uint64_t value) {
  return make_pattern(Pattern::Kind::Constant, ghidra::CPUI_COPY, value, -1, {}, false);
}

Pattern cst(int slot) {
  return make_pattern(Pattern::Kind::ConstantSlot, ghidra::CPUI_COPY, 0, slot, {}, false);
}
